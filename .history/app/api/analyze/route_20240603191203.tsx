...
const containsTailwindClasses = ($: cheerio.Root): boolean => {
  // More specific Tailwind class pattern
  const tailwindClassPattern = /\b(tw-[\w-]+|flex|grid|container|bg-(red|blue|green|yellow|indigo|purple|pink|gray|white|black|transparent|current)-\d{2,3}|text-(xs|sm|base|lg|xl|2xl|3xl|4xl|5xl|6xl|7xl|8xl|9xl|left|center|right|justify|opacity-\d{1,3})|rounded(-\w+)?|shadow(-\w+)?|p-\d|m-\d|w-\d|h-\d)\b/;
