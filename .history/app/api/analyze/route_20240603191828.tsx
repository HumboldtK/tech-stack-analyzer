...
const containsTailwindClasses = ($: cheerio.Root): boolean => {
  const tailwindClassPattern = /\b(tw-[\w-]+|bg-(red|blue|green|yellow|indigo|purple|pink|gray|white|black|transparent|current)-\d{2,3}|text-(xs|sm|base|lg|xl|2xl|3xl|4xl|5xl|6xl|7xl|8xl|9xl|left|center|right|justify|opacity-\d{1,3})|rounded(-\w+)?|shadow(-\w+)?|p-\d|m-\d|w-\d|h-\d|flex|inline-flex|grid-cols-\d+|gap-\d+|space-(x|y)-\d+|justify-(start|end|center|between|around|evenly)|items-(start|end|center|baseline|stretch))\b/;
  
  const bootstrapClassPattern = /\b(container|row|col|btn|card|alert|badge|breadcrumb|carousel|collapse|dropdown|form|input|modal|nav|navbar|pagination|popover|progress|spinner|toast|tooltip|jumbotron|text-center)\b/;

  return $('[class]').toArray().some(el => {
    const classList = $(el).attr('class') || '';

    // Exclude if Bootstrap class is present
    if (bootstrapClassPattern.test(classList)) {
      console.log('Bootstrap class matched:', classList);
      return false;
    }

    const tailwindMatch = classList.match(tailwindClassPattern);
    if (tailwindMatch) {
      console.log('Tailwind class matched:', tailwindMatch);
      return true;
    }
