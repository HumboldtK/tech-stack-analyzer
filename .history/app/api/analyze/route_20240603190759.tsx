...
const containsTailwindClasses = ($: cheerio.Root): boolean => {
  const tailwindClassRegex = /\b(tw-[a-zA-Z0-9-]+|flex|grid|container|bg-[a-zA-Z0-9-]+|text-[a-zA-Z0-9-]+|rounded-[a-zA-Z0-9-]*|shadow-[a-zA-Z0-9-]*)\b/;
  
  return $('[class]').toArray().some(el => {
    const classAttr = $(el).attr('class') || '';
    return tailwindClassRegex.test(classAttr);
  });
};
