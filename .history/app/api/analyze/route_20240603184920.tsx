...
const detectTechStack = async (html: string, headers: Record<string, string>, baseUrl: string): Promise<TechStack> => {
  ...
  if (await containsReact($, baseUrl)) jsFrameworks.add('React.js');
  ...
  const buildTools = [];
  if (html.includes('webpack')) buildTools.push('Webpack');
  if (await containsInScripts($, 'webpack', baseUrl)) buildTools.push('Webpack');
  if (buildTools.length > 0) techStack.buildTools = buildTools;
...
const containsReact = async ($: cheerio.Root, baseUrl: string): Promise<boolean> => {
  ...
  const isReactScriptPresent = dataReact.length > 0 || reactScripts.length > 0 || inlineReactScripts.length > 0;
...
const containsVue = async ($: cheerio.Root, baseUrl: string): Promise<boolean> => {
  const vueScripts = $('script[src*="vue"]');
  const inlineVueScripts = $('script').filter((i, el) => $(el).html()?.includes('Vue') ?? false);
...
const containsAngular = async ($: cheerio.Root, baseUrl: string): Promise<boolean> => {
  const angularScripts = $('script[src*="angular"]');
  const inlineAngularScripts = $('script').filter((i, el) => $(el).html()?.includes('ng') ?? false);
  const isAngularScriptPresent = angularScripts.length > 0 || inlineAngularScripts.length > 0;
  const isAngularScriptContentPresent = await containsInScripts($, 'angular', baseUrl, ['ng-', 'angular.io']);
...
const containsEmber = async ($: cheerio.Root, baseUrl: string): Promise<boolean> => {
  const emberScripts = $('script[src*="ember"]');
  const inlineEmberScripts = $('script').filter((i, el) => $(el).html()?.includes('Ember') ?? false);
  const isEmberScriptPresent = emberScripts.length > 0 || inlineEmberScripts.length > 0;
  const isEmberScriptContentPresent = await containsInScripts($, 'ember', baseUrl);
