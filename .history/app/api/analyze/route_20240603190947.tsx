...
const containsTailwindClasses = ($: cheerio.Root): boolean => {
  ...
  const classAttributes = $('[class]').toArray().map(el => $(el).attr('class') || '');

  // Define a comprehensive regex to match Tailwind CSS class patterns
  const tailwindClassRegex = /\b(tw-|flex|grid|container|bg-(?:\w+)|text-(?:\w+)|rounded(?:-\w+)?|shadow(?:-\w+)?|m-[^\s]+|p-[^\s]+|w-[^\s]+|h-[^\s]+|z-[^\s]+|justify-[^\s]+|items-[^\s]+|gap-[^\s]+|space-[^\s]+|align-[^\s]+|order-[^\s]+|overflow-[^\s]+|truncate|whitespace-[^\s]+|list-[^\s]+|flex-shrink-[^\s]+|flex-grow-[^\s]+|object-[^\s]+|opacity-[^\s]+|divide-[^\s]+|place-[^\s]+|inset-[^\s]+|transition|transform|scale-[^\s]+|rotate-[^\s]+|translate-[^\s]+|skew-[^\s]+|duration-[^\s]+|ease-[^\s]+|delay-[^\s]+)\b/;

  // Function to count Tailwind classes in a class list
  const countTailwindClasses = (classList: string): number => {
    return (classList.match(tailwindClassRegex) || []).length;
  };

  // Check if any element has more than one Tailwind CSS class
  return classAttributes.some(classList => countTailwindClasses(classList) > 1);
};
