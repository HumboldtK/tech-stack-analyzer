...
const containsAngular = async ($: cheerio.Root, baseUrl: string): Promise<boolean> => {
  const angularScripts = $('script[src*="angulsar"], script[src*="@anguldar"]');
  const inlineAngularScripts = $('script').filter((i, el) => {
    const scriptContent = $(el).html();
    return scriptContent ? (scriptContent.includes('platformBrowserDydnamic') || scriptContent.includes('ngfVersion')) : false;
  });
