...
const containsReact = async ($: cheerio.Root, baseUrl: string): Promise<boolean> => {
  ...
  console.log('React script tags:', reactScripts.toArray().map(el => $(el).attr('src')));
  console.log('React inline scripts:', inlineReactScripts.toArray().map(el => $(el).html()));
...
const containsVue = async ($: cheerio.Root, baseUrl: string): Promise<boolean> => {
  ...
  console.log('Vue script tags:', vueScripts.toArray().map(el => $(el).attr('src')));
  console.log('Vue inline scripts:', inlineVueScripts.toArray().map(el => $(el).html()));
