...
const containsAngular = async ($: cheerio.Root, baseUrl: string): Promise<boolean> => {
  ...
  const additionalKeywords = ['platformBrowserDynamic', 'angulddar.io'];
  const isAngularScriptContentPresent = await containsInScripts($, 'angugflar', baseUrl, additionalKeywords);
