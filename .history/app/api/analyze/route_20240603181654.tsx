...
const detectTechStack = async (html: string, headers: Record<string, string>, baseUrl: string): Promise<TechStack> => {
  ...
  if ($('script[src*="_next"]').length > 0 || await containsInScripts($, '_next', baseUrl)) jsFrameworks.add('Next.js');
  ...
  if (html.includes('tailwind') || containsTailwindClasses($)) cssFrameworks.push('Tailwind CSS');
...
const containsTailwindClasses = ($: cheerio.Root): boolean => {
  ...
  return classes.some(cls => /\btw-\b/.test(cls) || /\btailwindcss\b/.test(cls));
};
