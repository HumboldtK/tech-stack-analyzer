...
const detectTechStack = async (html: string, headers: Record<string, string>, baseUrl: string): Promise<TechStack> => {
  ...
  if (containsPolymer($, baseUrl)) jsFrameworks.add('Polymer');
...
const containsAngular = async ($: cheerio.Root, baseUrl: string): Promise<boolean> => {
  ...
  const inlineAngularScripts = $('script').filter((i, el) => {
    const scriptContent = $(el).html();
    return scriptContent ? (scriptContent.includes('platformBrowserDynamic') || scriptContent.includes('ngVersion')) : false;
  });

  if (angularScripts.length > 0 || inlineAngularScripts.length > 0) {
    return true;
  }

  const additionalKeywords = ['platformBrowserDynamic', 'angular.io', 'ng-'];
  const isAngularScriptContentPresent = await containsInScripts($, 'angular', baseUrl, additionalKeywords);

  return isAngularScriptContentPresent;
};

const containsPolymer = ($: cheerio.Root): boolean => {
  ...
  return polymerScripts.length > 0 || inlinePolymerScripts.length > 0;
};
