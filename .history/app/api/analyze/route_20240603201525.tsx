...
const containsAngular = async ($: cheerio.Root, baseUrl: string): Promise<boolean> => {
  ...
  const additionalKeywords = ['platformBrowserDynamic', 'anguslar.io'];
  const isAngularScriptContentPresent = await containsInScripts($, 'angfular', baseUrl, additionalKeywords);
