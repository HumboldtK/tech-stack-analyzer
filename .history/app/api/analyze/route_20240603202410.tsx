...
const detectTechStack = async (html: string, headers: Record<string, string>, baseUrl: string): Promise<TechStack> => {
  ...
  if (headers['server']) {
    ...
    if (serverHeaderMap[serverName]) {
      techStack.server = serverHeaderMap[serverName];
    } else if (serverName.includes('amazon')) {
      techStack.server = 'Amazon Web Server';
    } else {
...
const containsAngular = async ($: cheerio.Root, baseUrl: string): Promise<boolean> => {
  const angularSelectors = [
    'script[src*="angular/core"]',
    'script[src*="@angular/core"]',
    'script:contains("platformBrowserDynamic")'
  ];
  ...
  const additionalKeywords = ['platformBrowserDynamic'];
  const isAngularScriptContentPresent = await containsInScripts($, 'platformBrowserDynamic', baseUrl, additionalKeywords);
