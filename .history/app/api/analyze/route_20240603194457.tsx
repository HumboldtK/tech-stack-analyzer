...
const containsInScripts = async ($: cheerio.Root, keyword: string, baseUrl: string, additionalKeywords: string[] = [], matches: string[] = []): Promise<boolean> => {
...
const containsTailwindClasses = ($: cheerio.Root): boolean => {
  ...
  return $('[class]').toArray().some(el => {
    ...
    if (bootstrapClassPattern.test(classList)) {
      return false;
    }
...
const containsReact = async ($: cheerio.Root, baseUrl: string): Promise<boolean> => {
  ...
  const isReactScriptContentPresent = await containsInScripts($, '/react', baseUrl, ['react-dom.production.min.js', '@license React', 'reactjs.org'], reactMatches);

  if (isReactScriptPresent || isReactScriptContentPresent) {
    console.log("React detected:", {
      reactScripts: reactScripts.length,
      inlineReactScripts: inlineReactScripts.length,
...
const containsVue = async ($: cheerio.Root, baseUrl: string): Promise<boolean> => {
  ...
  const vueMatches: string[] = [];
  const isVueScriptContentPresent = await containsInScripts($, '/vue', baseUrl, ['Vue.config', 'vuejs.org'], vueMatches);

  if (isVueScriptPresent || isVueScriptContentPresent) {
    console.log("Vue detected:", {
      vueScripts: vueScripts.length,
      inlineVueScripts: inlineVueScripts.length,
      isVueScriptPresent,
