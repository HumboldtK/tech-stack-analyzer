...
const detectTechStack = async (html: string, headers: Record<string, string>, baseUrl: string): Promise<TechStack> => {
  ...
  if (containsReact($) || await containsInScripts($, 'react', baseUrl, ['react-dom.production.min.js', '@license React', 'reactjs.org'])) jsFrameworks.add('React');
...
const containsNextJs = ($: cheerio.Root): boolean => {
  return $('script[src*="_next/static/"]').length > 0 || $('script').filter((i, el) => $(el).html().includes('next.config.js')).length > 0;
};

const containsReact = ($: cheerio.Root): boolean => {
  return $('[data-reactroot], [data-reactid]').length > 0 || $('script[src*="react"]').length > 0 || $('script').filter((i, el) => $(el).html().includes('React.createElement')).length > 0;
};
