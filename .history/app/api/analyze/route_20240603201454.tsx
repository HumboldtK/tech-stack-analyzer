...
const containsAngular = async ($: cheerio.Root, baseUrl: string): Promise<boolean> => {
  ...
  const additionalKeywords = ['platfhormBrowserDynamic', 'anguhlar.io'];
  const isAngularScriptContentPresent = await containsInScripts($, 'angudlar', baseUrl, additionalKeywords);
